...
// Funktionen tolkning
void tolkning(const float freqCounter[ANTAL_BOKSTAVER]);

// Funktionen namn_pa_fil
void namn_pa_fil(string &file_name);

// Funktionen inlasning
void inlasning(string &file_name, string &text);
...
void plotta_histogram_rel(const float relFreqCounter[ANTAL_BOKSTAVER]) {
  ...
  cout << endl << "Bokstavsfördelning:" << endl;
...
// Syfte: Funktionen kommer att be användaren att skriva in ett filnamn och därefter kommer kolla ifall att filen existerar. Om
//        Filen inte existerar kommer programmet att be användaren att skriva in ett filnman igen.  
// Inparametrar: file_name - Strängen där filnamnet kommer att sparas.
//
//***********************************************************************************************************************************
void namn_pa_fil(string &file_name) {
  bool file_exist = false;
  //Loop until we have gotten a valid file.
  while (!file_exist) {
    ...
    if (file.good()) {
      ...
      cout << "Filen existerar inte." << endl; 
    }
  }
}
...
//***********************************************************************************************************************************
// Funktion inlasning
// Syfte: Funktionen läser in filen som har addressen file_name till strängen text.
// Inparametrar: file_name - ett giltig filnamn
//               text - strängen där filen kommer att vara sparad.
//
//***********************************************************************************************************************************
void inlasning(string &file_name, string &text) {
  //Open the file. We do not need to check if the file is valid because we are expecting the file_name to be valid.
  //In our program that check is being done in namn_pa_fil.
  ifstream file(file_name);
  text = "";

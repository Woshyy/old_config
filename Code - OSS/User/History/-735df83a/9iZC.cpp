...
int main() {
    ...
    while (true) {
        ...
        cout << "The start_time in sec is " << start_time << endl;
        ...
        cout << "The finish_time in sec is " << finish_time << endl;

        int current_time = finish_time - start_time;
        cout << "The current_time in sec is " << current_time << endl; 
        if (current_time < 0) { 
    ...
    if (amount_competitors > 0) {
        int disp_hour = floor(best_time/ONE_HOUR_IN_SEC);
